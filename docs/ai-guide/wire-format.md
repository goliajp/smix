...
`Input-Dispatch-Mode` used to be listed here. The client sends it and
`--force-key-events` sets it, but no runner has ever read it — see the
note under `POST /fill`.
...
> **`Input-Dispatch-Mode` is not implemented.** The client sends the
> header and `smix run --force-key-events` sets it, but neither runner
> reads it, so the flag does not change how text is dispatched. It was
> meant for RN apps whose hidden `<TextInput>` defeats a11y-focus
> lookup; until a runner honours it, reach the field by coordinate
> (`tapOn: { point: "X%,Y%" }`) and then use the scalar `inputText`,
