...
- **第一次跑完,立刻把真实数字写回本文件此处**:`tests= / failures= / errors= / skipped=`
  ...
  **实测记账(2026-07-21,`sim-smix-android-01` / android-33 / arm64-v8a):**

  ```
  tests="4" skipped="0" failures="0" errors="0"   time=0.283s
  ```

  | 测试 | 结果 |
  |---|---|
  | `dev.smix.sdk.Spike001Test#testEmptyTreeIdMissByteIdenticalToRust` | pass |
  | `dev.smix.sdk.Spike001Test#testInvalidTreeJsonThrows` | pass |
  | `dev.smix.sdk.Spike001Test#testInvalidSelectorJsonThrows` | pass |
  | `dev.smix.sdk.ConformanceHarnessTest#emitAllFixtureOutputsToInstrumentCache` | pass(**设计上恒 pass**) |

  **失败清单:空。没有 stale 断言要修** —— 冷计划"从未跑过的断言大概率 stale"的担忧
  在 `:sdk` 这一份上不成立。红向注入(把首条的期望值改成 `listOf("INJECTED-FAILURE")`)
  得 `tests="4" failures="1"`,XML 精确点名该测试,证明这 4 个数不是 `OK (0 tests)` 的伪绿。

  **但这份"全绿"的含金量必须写清**:4 条里 1 条是恒 pass 的 emit harness,而它 emit 的
  东西**没有任何东西去读** —— 头注释点名的 `pull-android-conformance.sh` 与
  `sync-conformance-fixtures-to-android-assets.sh` **从来不存在**,
  `run-cross-binary-harness.sh` 也没接 Kotlin 后端。所以真正被断言的只有 Spike001 的
  3 条。已改注释说明此事(注释是断言,代码才是事实)。fixture 数 **24**(两侧 `diff -rq`
  一致,assets 侧多一个 `.gitkeep`),冷计划写的 26 已更正。
...
- 把 A/B 两条的实际输出写回本文件此处。
  ...
  **实测(2026-07-21):**
  ...
  - **证据 A**:`adb -s emulator-5554 forward tcp:28080 tcp:28080` 后
    `/health` = **200**,body =
    `{"ok":true,"status":"ok","runner":"smix-android-runner","version":"2.0.0","runnerVersion":"2.0.0"}`
    → 它在正常服务,不是卡在安装或启动。
  - **证据 B**:gradle 进程(pid 96293)在 **3 分 40 秒**后 `kill -0` 仍 exit 0,
    gradle 输出自始至终停在
    `sim-smix-android-01(AVD) - 13 Tests 0/1 completed. (0 skipped) (0 failed)`。
    **一个只有 1 个 `@Test` 的套件,0 完成,同时端口在服务** —— 两条证据合起来只能读成
    一件事:这个任务不会返回。
  - 收尾:`kill` 后 gradle 未立即退出,补 `kill -9`;`am force-stop dev.smix.runner.test`
    + `forward --remove tcp:28080` 后 28080 返回 `000`(不通),确认两头都收干净。

  **结论(交给 C3 的硬约束)**:`:app:connectedDebugAndroidTest` 接进 `ship.sh` = 发布脚本
  永久挂起。C3 不得对 `:app` 套用 `:sdk` 的接法。
...
- `/tree` 若返回空树或报错,如实记下并定性,**不在本段修**——那是 C4 的行为 smoke,
  ...
  **实测(2026-07-21):**
  ...
  - **对计划的必要偏离**:计划写的是 `smix runner up …`,即 PATH 上的那个 —— 本机
    `/Users/doracawl/.local/bin/smix` 是 **1.0.27**,而仓库是 2.0.0。拿旧二进制跑等于
    测了另一个产品(也不含当天新加的 flag 拒绝)。改用 `./target/debug/smix`
    (`cargo build -p smix-cli --bin smix`,实测 `smix 2.0.0`)。**同一类风险以后都在:
    计划里出现裸 `smix` 时,先确认 PATH 上那个是不是这棵树构建出来的。**
  - `runner up emulator-5554 --platform android` → exit **0**,
    `runner up: http://localhost:28080/health = 200`
  - `/health` = **200**;`/tree` = **200,25483 字节**,根节点
    `{"rawType":"android.view.WindowRoot","bounds":{...w:1080,h:2340},...}`,
    子树含 `navigation_bar_frame` 等系统窗口 —— **不是空树**,a11y 抓取通路是活的
    (当时 emulator 前台无被测应用,所以树里只有系统窗口,符合预期)
  - `runner down --platform android --device emulator-5554` → exit **0**,
    `port 28080 closed`,复查 `/health` 返回 `000`(不通)
  - **顺带在真二进制上验证了当天的 flag 拒绝**:
    `runner up emulator-5554 --platform android --supervise` → exit **1**,
    输出即那条具名消息,且**在任何设备写入之前**返回。
  - **本段未追、如实记账的噪音**:每次 CLI 调用会把
    `kevy: AOF … replayed 6 commands …` 打进用户可见输出,同一行重复 3 次。
    属 v2.2 边界外(§8.1),记在此处待定。
