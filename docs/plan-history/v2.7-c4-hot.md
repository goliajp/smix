...
## 归档记录(2026-07-23,C4 通过)

**两步都做完了,而两步的口径都被设备推翻过一次。**

- **S1(#2)**:口径 1 写着「runner 一行不用改」。**错**。主机算不出按压真正开始的时刻(解析元素在按下之前),要么 runner 报边界,要么只能猜。改成 runner 报边界之后,数字又推翻了更上游的东西:`press(forDuration:)` 对 500ms 到 6000ms 的每一个请求都恒定 ~2.6s,`duration:` 从未生效。改走自己编写时间轴的合成路径。
- **S2(#8)**:口径 2 问「附着是什么意思」,答案是**这个语义早就实现了,只是没人接线** —— `LaunchModeResolver` 认 `activate`,Rust 侧零命中。补的是那根线,不是语义。
- **额外**:让 runner 答新路由的过程中撞见 `smix runner up` 发出去的 Swift 跟仓库里的不是一份;补 `tarball_is_current.rs`,它一红就照出 `HitChain.swift` / `TouchTimeline.swift` 从未进过 tarball。

细节见 `docs/v2.md` 决策日志 2026-07-23 三条。
