...
|---|---|---|---|---|---|---|---|---|
| N1 | `05-cli.md` §Environment-variable precedence | 端口取值走「flag → env → registry → default」四级 | broken | `the_registry_rung_is_still_unreachable_from_single_shot_verbs` | `smix run` 四级齐全:`--runner-port` 带 `env = "SMIX_RUNNER_PORT"`(clap 在 `main.rs:2672 run_port` 之前就把 env 并进 flag 了),`--device` 是索引注册表的键。单点 verb(`tap` / `find` / `wait-for` / `fill` / `press-key` / `scroll` / `tree` / `describe` / `system-popups` …)是 `--port` → `act.rs:34 runner_port_from_env`(env → 常量)—— **flag 与 env 都在,缺的是 registry 一级,而且它们没有任何参数能命名设备**,于是没有键可查 | cli+docs | — | 2026-07-22 |
| N2 | `08-cookbook.md` `apps.yaml` 示例 | Android 启动 Activity 可由 `activity:` 配置 | broken | `a_configured_launch_activity_still_reaches_nothing` | `apps_config.rs:69` 收下 `activity` 并给默认值 `.MainActivity`,`ResolvedApp::Android` 也带着它;而 `apps_config.rs:179 resolve_app_into_flow` 只用 `primary_id()`(包名),**没有任何调用方读 activity**。设备侧由 `android-runner/app/src/main/kotlin/dev/smix/runner/RunnerWire.kt:157` 钉死 `$bundleId/.MainActivity` | parser+kotlin-runner+docs | — | 2026-07-22 |
