...
| ⑤a | iOS `/tap` 只解 `selector.text`,Id/Label/Role/regex 选择器解不出来 | present | `at crates/smix-driver/src/lib.rs:477 "require_plain_text_selector"` | **C1 把这条记错了**:`tapOn: {id}` 不走这条路(默认走主机侧解树 + `/tap-at-norm-coord`)。唯一 POST `/tap` 的是 `dispatch: daemonProxy`,而它在上 wire 前就被这个 guard 拦下并给出具名错误 + 替代路径 —— 不是 400 烧 5s,是**能力边界**:daemonProxy 分发只吃纯文本选择器,RN 的 `testID` 用不上。需动 Swift runner 路由,留需设备段 | swift-runner | 2026-07-22 |
...
| ⑦ | `--retry` attribution 的 errorClass 映射与 adapter 退出码表不符 | fixed | `at crates/smix-cli/src/main.rs:1634 "PARSE_ERROR"` | 2=PARSE_ERROR / 3=EXPECTATION_FAILURE / 4=UNKNOWN_VERB / 5=FLOW_IO_ERROR / 6=RUNNER_UNREACHABLE,与 adapter 的 return 码逐码对齐;旧表把 parse 错归因成 timeout | — | 2026-07-22 |
| ⑧ | `smix run` 不读注册表 `runnerPort` | fixed | `at crates/smix-cli/src/main.rs:2670 "fn run_port"` | **C1 把这条记错了**:`smix run` 自 07-19 起就是 flag/env → `lookup_registered` → 22087,C1 只读了服务单点 verb 的 `runner_port_from_env` 就下结论、没跟到 `Cmd::Run` 分支。本段没修代码,补了 4 条断言(含「有 flag 时不读注册表」的惰性)并把行改真 | — | 2026-07-22 |
| ⑨a | `smix annotate` 帮助示例与实现不符 | fixed | `at crates/smix-cli/src/main.rs:1830 ""circle" =>"` | 帮助里的 `circle,at:100_100,color:red,radius:40` 实测越过 spec 解析(报错停在 decode input PNG) | — | 2026-07-22 |
