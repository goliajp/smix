...
class ChunkedLoopTest {
    ...
    private fun run(text: String, drops: Map<Int, (String) -> String>, before: String = "") =
        Field(before, drops).let { f ->
            f to RunnerWire.typeInChunks(text, before, masked = false, chunkPoints = 4, retypes = 3) { sent, _, _ ->
                f.type(sent)
            }
        }
