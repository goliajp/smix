...
THEIRS_AVD="${SMIX_C5_THEIRS_AVD:-$E2E_ANDROID_THIRD}"
...
OURS_SERIAL=""
cleanup() {
  [ -n "$THEIRS_ANDROID" ] && { e2e_stop_emulator "$THEIRS_ANDROID" || true; }
...
# "Theirs" is a read-only instance of a smix AVD started by hand. The
# emulator refuses that while the same AVD runs normally anywhere ("run
# all emulators with -read-only"). It was the first smix AVD — the one a
# release is driving — so in a release this could only say it could not
# judge; the third AVD exists for this and nothing else drives it. The
# check stays: somebody may have started it.
for s in $(adb devices 2>/dev/null | awk '$1 ~ /^emulator-/ && $2=="device" {print $1}'); do
...
e2e_start_emulator "$WORK/ours-hand.log" -avd "$OURS_AVD" -port "$OURS_PORT" -no-boot-anim
...
e2e_stop_emulator "emulator-$OURS_PORT" || fail "the registration boot of $OURS_AVD did not stop"
...
e2e_start_emulator "$WORK/theirs-android.log" -avd "$THEIRS_AVD" \
  -port "$THEIRS_PORT" -no-boot-anim -read-only
...
[ "$(simulator_state "$THEIRS_IOS")" = Booted ] \
