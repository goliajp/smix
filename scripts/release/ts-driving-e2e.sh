...
# Before anything reads it: this line used to come after the port
# helper below, so under `set -u` the script ended on its first
# `$ROOT` and never reached a device.
