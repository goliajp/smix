...
/// Host-side probe for `AccessibilityPlatformTranslation.framework`.
///
...
///
/// dlsym-only invariant: framework is **not** `linkedFramework` in
/// Package.swift and **not** imported as a Swift module — load is via
/// `Darwin.dlopen(...,RTLD_NOW | RTLD_GLOBAL)`. `RTLD_GLOBAL` is required
/// so the dyld registers the AXP ObjC class metadata before
/// `NSClassFromString` is called.
public enum AxpCapabilityProbe {
  ...
  /// 5 ObjC classes that the host-side AXP tree-read path will need.
  ...
  /// host-side AXP tree-read path will eventually invoke. This probe only
  /// checks `responds(to:)` — does **not** invoke any of them (XPC routing
  /// requires a registered `bridgeTokenDelegate`, which is out of scope
  ...
  /// the freshly-mapped ObjC classes.
  ///
  ...
  /// for parity with `IndigoSymbols.resolve(...via:)` so future wiring
  ...
  public static func probeLive(via resolver: DlsymResolver) -> AxpCapabilityReport {
    ...
    // registered bridgeTokenDelegate which we do NOT install).
