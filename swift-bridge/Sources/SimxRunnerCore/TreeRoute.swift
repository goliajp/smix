...
// Pure functions on a POCO (`A11ySnapshotData`) so SimxRunnerCore stays free
...
public enum TreeRoute {
  ...
  /// XCUI snapshot tree depth cliff. Apple's XCUITest hard-limits snapshots
  /// at ~60 levels (deeply nested RN/Flutter trees fail entirely above this).
  /// C1 silently truncates at MAX_DEPTH and logs once via the injected
  /// logSink; the A11yNode schema does NOT carry a _truncated flag — that
  /// decision is deferred to C3 protocol freeze.
  public static let MAX_DEPTH: Int = 60
  ...
  private static func nodeToDict(
    ...
    // C1 placeholder — XCUIElementSnapshot does not expose hasFocus.
    // The AccessibilityPlatformTranslation primary path (C2-C3) will fill it.
    out["hasFocus"] = false
