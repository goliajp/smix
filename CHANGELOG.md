...
- **Animations are quietened by default** — a run now asks the device to stop animating before it foregrounds the app, and reads the setting back to check it took. How far it gets differs by platform and the wording does not paper over it: Android's `window`, `transition` and `animator` scales go to zero, which is off; iOS gets the Reduce Motion accessibility setting, which is weaker but is as far as smix can push on its own — XCUITest runs in a separate process from the app under test, so `UIView.setAnimationsEnabled(false)` cannot reach it. `--animations` runs with the device's own settings instead. **If you record `assertScreenshot` baselines, they were captured with animations running and may no longer match**; pass `--animations` on those flows or re-record. `waitForAnimationToEnd` still earns its place on iOS, where Reduce Motion is not the same as zero duration; on Android it now has little left to wait for.
