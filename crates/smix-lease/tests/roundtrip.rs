...
use smix_lease::store::{self, LeaseError};
...
fn write_then_read_roundtrips() {
    ...
    store::write(tmp.path(), &l).expect("write");
    let back = store::read(tmp.path(), "UDID-A")
        .expect("read")
        .expect("some");
    assert_eq!(back, l);
}
...
fn release_is_idempotent() {
    ...
    store::write(tmp.path(), &l).expect("write");
    store::remove(tmp.path(), "UDID-B").expect("first remove");
    store::remove(tmp.path(), "UDID-B").expect("second remove");
    assert!(store::read(tmp.path(), "UDID-B").expect("read").is_none());
}

#[test]
fn corrupt_ledger_is_loud_not_treated_as_absent() {
    let tmp = tempfile::tempdir().expect("tmpdir");
    let path = store::lease_path(tmp.path(), "UDID-C").expect("path");
    std::fs::create_dir_all(path.parent().expect("parent")).expect("mkdir");
    std::fs::write(&path, b"{\"deviceId\": \"UDID-C\", \"holder\"").expect("write junk");
    match store::read(tmp.path(), "UDID-C") {
...
fn device_id_that_is_not_a_filename_is_refused() {
    ...
    for bad in ["../escape", "a/b", ""] {
        match store::lease_path(tmp.path(), bad) {
...
fn facts_from_a_dead_holder_reclaim_with_cleanup() {
    ...
    store::write(tmp.path(), &l).expect("write");
    let facts = store::collect_facts(tmp.path(), "UDID-D").expect("facts");
...
fn a_second_resource_of_the_same_kind_replaces_the_first() {
    ...
    store::add_resource(tmp.path(), "UDID-E", first).expect("first");
    store::add_resource(tmp.path(), "UDID-E", second).expect("second");
    let lease = store::read(tmp.path(), "UDID-E")
        .expect("read")
        .expect("some");
...
fn dropping_the_last_meaningful_row_clears_the_ledger() {
    ...
    store::add_resource(tmp.path(), "UDID-F", Resource::Booted { by_us: false }).expect("boot");
    ...
    .expect("drop");
    assert!(
        store::read(tmp.path(), "UDID-F").expect("read").is_none(),
...
fn a_boot_we_performed_keeps_the_ledger_alive_on_its_own() {
    let tmp = tempfile::tempdir().expect("tmpdir");
    store::add_resource(tmp.path(), "UDID-G", Resource::Booted { by_us: true }).expect("boot");
    store::drop_resource_kind(
        tmp.path(),
        "UDID-G",
        &Resource::Runner {
    ...
    .expect("drop");
    let lease = store::read(tmp.path(), "UDID-G").expect("read");
