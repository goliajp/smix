...
///
/// Why this line and not `topResumedActivity=`, measured on API 36
/// (2026-09-24): `ResumedActivity:` is printed once per display, at
/// display level, and named the same activity as `dumpsys window`'s
/// `mFocusedApp` and `mCurrentFocus` both fullscreen and in
/// `--windowingMode 6`; `topResumedActivity=` is printed inside each
/// task's section. The runner bring-up in smix-capsule had its own
/// reader preferring the other line, and a test pinning that — the two
/// are one reader now, so `runner up` and `smix sim frontmost` cannot
