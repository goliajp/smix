...
//!
//! The DEFAULT clear-state semantic is in-place
//! (`Terminate + PrivacyResetAll + SandboxClearInPlace + Launch`) to
//! avoid the iOS 26.5 XCUITest binding loss and the
//! ReportCrash "<app> quit unexpectedly" dialog that
...
fn plan_some_path_clear_state_default_is_in_place() {
    ...
    // One advisory warning is emitted when app_path is set but
...
fn plan_none_path_clear_state_default_is_in_place() {
    // When app_path is missing on the default path we
